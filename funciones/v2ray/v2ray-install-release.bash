...
systemd_cat_config() {
  if systemd-analyze --help | grep -qw 'cat-config'; then
    ...
    cat "$@" "$1".d/*
    echo "${aoi}~~~~~~~~~~~~~~~~"
    warning "La version instalada de systemd es demasiada vieja."
    warning "Considere actualizarla a la última versión."
    echo
  fi
}
  ...
  if [[ "$(uname)" == 'Linux' ]]; then
    case "$(uname -m)" in
      ...
      's390x')
        ...
        ;;
      *)
        error "No se pudo identificar el sistema operativo y el arquitectura."
        exit 1
        ;;
    esac
    if [[ ! -f '/etc/os-release' ]]; then
      error "No utilizando distribución anticuadas."
      exit 1
    fi
    ...
    if [[ -f /.dockerenv ]] || grep -q 'docker\|lxc' /proc/1/cgroup && [[ "$(type -P systemctl)" ]]; then
      ...
      error "Solo se admiten las distribuciones de Linux utilizando Systemd."
      exit 1
    fi
  ...
  while [[ "$#" -gt '0' ]]; do
    case "$1" in
      '--remove')
        ...
        ;;
      '--version')
        VERSION="${2:?error: Especifique la version correcta .}"
        break
        ;;
      '-c' | '--check')
        CHECK='1'
      ...
      '-l' | '--local')
        LOCAL_INSTALL='1'
        LOCAL_FILE="${2:?error: Por favor, especifique el archivo local correcto.}"
        break
        ;;
      '-p' | '--proxy')
        if [[ -z "${2:?error: Por favor, especifique la dirección del servidor proxy.}" ]]; then
          exit 1
        fi
        PROXY="$2"
        shift
...
install_software() {
  ...
  if ${PACKAGE_MANAGEMENT_INSTALL} "$package_name"; then
    info "$package_name ya se encuetra instalado"
  else
    error "La instalacion de $package_name fallo"
    exit 1
  fi
}
...
get_version() {
  ...
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$TMP_FILE" 'https://api.github.com/repos/v2fly/v2ray-core/releases/latest'; then
    "rm" "$TMP_FILE"
    error "Fallo al obtener la lista de versiones de V2Ray."
    exit 1
  fi
...
download_v2ray() {
  ...
  info "Descargando v2ray: $DOWNLOAD_LINK"
  if ! curl -x "${PROXY}"  -R -H 'Cache-Control: no-cache' -o "$ZIP_FILE" "$DOWNLOAD_LINK"; then
    error "La descarga fallo. Compruébelo su conexión a internet."
    return 1
  fi
  info "Descargando el archivo de verificacion de v2ray: $DOWNLOAD_LINK.dgst"
  if ! curl -x "${PROXY}" -sSR -H 'Cache-Control: no-cache' -o "$ZIP_FILE.dgst" "$DOWNLOAD_LINK.dgst"; then
    error "La descarga fallo. Compruébelo su conexión a internet."
    return 1
  fi
  if [[ "$(cat "$ZIP_FILE".dgst)" == 'Not Found' ]]; then
    error 'Esta versión no admite la verificación.Por favor, reemplace con otra versión..'
    return 1
  fi
  ...
  for LISTSUM in 'md5' 'sha1' 'sha256' 'sha512'; do
    ...
    if [[ "$SUM" != "$CHECKSUM" ]]; then
      error 'La verificacion fallo. Compruebe su conexión a internet.'
      return 1
    fi
  done
}
...
decompression() {
  if ! unzip -q "$1" -d "$TMP_DIRECTORY"; then
    error "Fallo al descomprimir v2ray."
    "rm" -r "$TMP_DIRECTORY"
    echo "removed: $TMP_DIRECTORY"
    exit 1
  fi
  info "Extrayendo el paquete V2Ray en $TMP_DIRECTORY, y preparando para instalar."
}
...
install_startup_service_file() {
  ...
  if [[ -n "$JSONS_PATH" ]]; then
    ...
    echo "# En caso de que tenga una buena razón para hacerlo, duplique este archivo en el mismo directorio y haga su personalización allí.
# O todos los cambios que hagas se perderán.!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
...
ExecStart=/usr/local/bin/v2ray -config ${JSON_PATH}/%i.json" > \
  ...
  info "Los archivos de servicio systemd se han instalado con éxito!"
  warning "Los siguientes son los parámetros para el inicio del servicio V2Ray."
  warning "Asegúrese de que la ruta del archivo de configuración esté correctamente configurada."
...
start_v2ray() {
  if [[ -f '/etc/systemd/system/v2ray.service' ]]; then
    if systemctl start "${V2RAY_CUSTOMIZE:-v2ray}"; then
      info "El servicio V2Ray se ha iniciado con éxito!"
    else
      error "Fallo al iniciar el servicio V2RAY!"
      exit 1
    fi
  fi
}
...
stop_v2ray() {
  ...
  if ! systemctl stop "$v2ray_daemon_to_stop"; then
    error "Fallo al detener el servicio V2Ray!"
    exit 1
  fi
  info "El servicio V2Ray se ha detenido con éxito!"
}
...
check_update() {
  if [[ -f '/etc/systemd/system/v2ray.service' ]]; then
    ...
    if [[ "$get_ver_exit_code" -eq '0' ]]; then
      info "Se encontró una nueva versión de V2Ray: $RELEASE_VERSION . (Versión actual: $CURRENT_VERSION)"
    elif [[ "$get_ver_exit_code" -eq '1' ]]; then
      info "No se encontró una nueva versión de V2Ray. La version actual es la $CURRENT_VERSION."
    fi
    exit 0
  else
...
remove_v2ray() {
  if systemctl list-unit-files | grep -qw 'v2ray'; then
    ...
    if ! ("rm" -r '/usr/local/bin/v2ray' \
      ...
      echo 'Eliminado: /usr/local/bin/v2ray'
      echo 'Eliminado: /usr/local/bin/v2ctl'
      echo "Eliminado: $DAT_PATH"
      echo 'Eliminado: /etc/systemd/system/v2ray.service'
      echo 'Eliminado: /etc/systemd/system/v2ray@.service'
      echo 'Eliminado: /etc/systemd/system/v2ray.service.d'
      echo 'Eliminado: /etc/systemd/system/v2ray@.service.d'
...
show_help() {
  ...
  echo '  --remove        ELiminar v2ray'
  echo '  --version       Instalar una version especifica de v2ray, e.g., --version v4.18.0'
  echo '  -c, --check     Comprueba si hay actualizaciones de v2ray'
  echo '  -f, --force     Forzar la actualización de v2ray'
  echo '  -h, --help      Mostrar este mensaje de ayuda'
  echo '  -l, --local     Instalar v2ray de un archivo local'
  echo '  -p, --proxy     Descargar a travez de un servidor proxy, e.g., -p http://127.0.0.1:8118 or -p socks5://127.0.0.1:1080'
  exit 0
}
...
vray_main() {
  ...
  if [[ "$LOCAL_INSTALL" -eq '1' ]]; then
    warning "Instalando V2Ray desde un archivo local, pero aún necesita tener conexion a internet"
    warning "Por favor, asegúrese de que el archivo sea válido.(Presiona cualquier tecla) ..."
    read -r
    ...
    if [[ "$NUMBER" -eq '0' ]] || [[ "$FORCE" -eq '1' ]] || [[ "$NUMBER" -eq 2 ]]; then
      info "Instalando V2Ray $RELEASE_VERSION ..."
      download_v2ray
      if [[ "$?" -eq '1' ]]; then
        "rm" -r "$TMP_DIRECTORY"
  ...
  info "V2Ray $RELEASE_VERSION instalado correctamente."
